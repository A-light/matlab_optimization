...
tic;
t0=clock;
Rollerbearing_stress_sub_cnt=0;
...
for i=1:number_roller  %滚动体个数   
    if abs(Q_global(i,1))>1
        ...
        Rollerbearing_stress_sub_cnt=Rollerbearing_stress_sub_cnt+1;
       
    end
end
disp([num2str(delta_cnt)]);
disp(['a_cnt',num2str(a_cnt)]);
disp(['D_cnt',num2str(D_cnt)]);
disp(['total times:',num2str(Rollerbearing_stress_sub_cnt)]); 
disp(['程序总运行时间：',num2str(etime(clock,t0))])

%disp(stress_outer);
%disp(stress_inner);
%disp(half_width_outer)
%disp(half_width_inner)
end
%%%程序结束
...
%delta_interval_outer
%种群的大小
popsize=10;
delta_set=init_delta(popsize,delta_initial);
delta_outer_set=delta_set;
...
[delta_outer_set,right]=find_init_dalta(R_total_outer,hj,yyj,...
            z_total_outer,number_slice,E_roller_outer,delta_outer_set,force,popsize,tolerance);
if right~=0
    delta_outer=delta_outer_set(right);
    [contact_force_outer,contact_stress_outer,a_i_outer,a_i_outer_first,a_i_outer_last,mark_outer]=linear_contact_inter(R_total_outer,hj,yyj,...
                z_total_outer,number_slice,E_roller_outer,delta_outer);
else
    delta_outer=delta_outer_set(1,1);
    [contact_force_outer,contact_stress_outer,a_i_outer,a_i_outer_first,a_i_outer_last,mark_outer]=linear_contact_inter(R_total_outer,hj,yyj,...
                z_total_outer,number_slice,E_roller_outer,delta_outer);
    force_dif=(contact_force_outer-force)/force;
    while abs(force_dif)>tolerance
        for i=2:popsize
            delta_outer=delta_outer_set(i,1);
            [contact_force_outer,contact_stress_outer,a_i_outer,a_i_outer_first,a_i_outer_last,mark_outer]=linear_contact_inter(R_total_outer,hj,yyj,...
                z_total_outer,number_slice,E_roller_outer,delta_outer);
        ...
        delta_outer_set=find_init_dalta(R_total_outer,hj,yyj,...
                z_total_outer,number_slice,E_roller_outer,delta_outer_set,force,popsize,tolerance);
        if flag==1 
            break;
        end 
...
%切片受载变形向量（mm）
[contact_force_inner,contact_stress_inner,a_i_inner,a_i_inner_first,a_i_inner_last,mark_inner]=linear_contact_inter(R_total_inner,hj,yyj,...
            z_total_inner,number_slice,E_roller_inner,delta_inner);
delta_inner_set=delta_set;
%定义滚动体与内圈初始法向位移迭代变化值（mm），delta_interval_inner
flag=0;
[delta_inner_set,right]=find_init_dalta(R_total_inner,hj,yyj,...
            z_total_inner,number_slice,E_roller_inner,delta_inner_set,force,popsize,tolerance);
if right~=0
    delta_inner=delta_inner_set(right,1);
    [contact_force_inner,contact_stress_inner,a_i_inner,a_i_inner_first,a_i_inner_last,mark_inner]=linear_contact_inter(R_total_inner,hj,yyj,...
                z_total_inner,number_slice,E_roller_inner,delta_inner);
else
    [contact_force_inner,contact_stress_inner,a_i_inner,a_i_inner_first,a_i_inner_last,mark_inner]=linear_contact_inter(R_total_inner,hj,yyj,...
    ...
    while abs(force_dif)>tolerance&&right==0
        for i=2:popsize
            delta_inner=delta_inner_set(i,1);
            [contact_force_inner,contact_stress_inner,a_i_inner,a_i_inner_first,a_i_inner_last,mark_inner]=linear_contact_inter(R_total_inner,hj,yyj,...
                z_total_inner,number_slice,E_roller_inner,delta_inner);
            force_dif=(contact_force_inner-force)/force;
            if abs(force_dif)<tolerance
...
%%%程序结束
disp(['subfunction:Rollerbearing_stress_sub:滚动体与内圈接触法向力（N），contact_force_inner,滚动体与外圈切片接触应力向量（MPa），contact_stress_outer 滚动体与内圈切片实际接触半宽向量:',num2str(etime(clock,t1))]);
end
%%%程序结束
...
if sum(a_i>0)==0  %给定法向位移时初始状态滚动体无切片接触 ?
    ...
    while (max(abs(a_i_dif))>tolerance_a_i) %two while + two for
...
%disp(['滚动体接触法向力（N），contact_force 切片接触应力向量（MPa），contact_stress 切片实际接触半宽向量(mm)，a_i 实际接触区最左边切片的序号，a_i_first 实际接触区最右切片的序号，a_i_last,初始状态滚动体切片接触状态，mark_initial(time):',num2str(etime(clock,t2))]);
end
%%%程序结束
...
m=a_i_first-1;
mm=0;
for j=a_i_first:a_i_last %two for 
    m=m+1;
    mm=mm+1;
    yj=yyj(m,1);
...
S=S+feval(f,aj);
D_ij=0.5*h_sim*S;
%disp(['滚动体的接触柔度矩阵，D_ij,时间：',num2str(etime(clock,t4))])
end
%%%程序结束

 %a_i=0.5*(a_i+a_i_old);   %!!!!!!!!! crossover
 % 染色体变异结束

function [a_i]=crossover(a_i,a_i_old)
    global delta_cnt;
    %M=30
    a=rand([30,1])*1.5-0.25;
    a_i=a_i_old+a.*(a_i-a_i_old);
end

function [a]=init(a_i,num)
    a=rand([30,num])*max(a_i);
end
%


%二元竞标赛
function [force1,direction1,loc1,force2,direction2,loc2]=tourselect(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta,force,popsize) %两个个体在不同方向时
    loopsize=popsize/2;
    rd=randperm(popsize)
    indi1=rd(1);
    force1=fitness(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta_set(indi1,1));
    while loopsize>1
        indi2=rd(loopsize);
        force2=fitness(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta_set(indi2,1));
        [force1,direction1,loc1]=compare_force(force1,force2,force);
        if loc1==2
            loc1=indi2; %这里的loc表示确定个体所在的位置
        else
            loc1=indi1;
        end 
        loopsize=loopsize-1;
    ...
    while loopsize>0
        indi3=rd(loopsize);
        force3=fitness(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta_set(indi3,1));
        [force2,direction2,loc2]=compare_force(force2,force3,force);
        if loc2==2
            loc2=indi3; %这里的loc表示确定个体所在的位置
        else
            loc2=indi2; %这里的loc表示确定个体所在的位置
        end 
        loopsize=loopsize-1;
    end 
...
%交叉
function[delta_set]=cross_delta(delta_set,loc1,loc2,popsize) 
    a=rand(1)*1.5-0.25;%这里方式是不是可以改一下
    for i=i:popsize 
        delta_set(i,1)=delta_set(loc1,1)+a*(delta_set(loc2,1)-delta_set(loc1,1));
    end 
end 
%变异
function[delta_set] =mutation(delta_set,popsize) 
    rd=randperm(1000);
    for i=1:popsize
        if rd(1)<=10
           if mod(rd(1),2)==1 
                delta_set(i,1)=delta_set(i,1)+rand()*(max(delta_set)-delta_set(i,1))
            else
                delta_set(i,1)=delta_set(i,1)-rand()*(delta_set(i,1)-min(delta_set))
            end
        end 
    end
end
%求适应度
function [contact_force]=fitness(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta)
     [contact_force,contact_stress,a_i,a_i_first,a_i_last,mark]=linear_contact_inter(R_total,hj,yyj,z_total,number_slice,E_syn,delta);
end 
%相互比较
function [cforce,direction,loc]=compare_force(force1,force2,force)
    %loc用来标记哪个个体被选择了
    space1=force-force2;
    space2=force-force1;
    direction=1;
    if abs(space1)<=abs(space2)
        cforce=force1;
        if space1<0
            direction=0;
        end 
...
function [delta_set]=init_delta(popsize,delta_initial)
    delta_set=ones(popsize,1);
    delta_set=ones(popsize,1);
    delta_interval=delta_initial/10;
    for i=1:(popsize/2)
        delta_set(i,1)=delta_initial;
        delta_initial=delta_initial+delta_interval;
    end
    for i=(popsize/2):popsize
        delta_set(i,1)=delta_initial;
        delta_initial=delta_initial-delta_interval;
    end
end 

function [delta_set,right]=find_init_dalta(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta_set,force,popsize,tolerance)
    delta_route=ones(popsize,1);
    for i=1:popsize
        contact_force=fitness(R_total,hj,yyj,...
                 z_total,number_slice,E_syn,delta_set(i,1));
        if contact_force<force
            delta_route(i,1)=0;
        end 
    end
    judgment=sum(delta_route);
    flag=0;
    right=0;
    while (judgment==popsize||judgment==0)&&flag==0
        while judgment==popsize&&flag==0
            delta_route=ones(popsize,1);
            delta_set=init_delta(popsize,delta_set(1,1)/2);
            for i=1:popsize
                contact_force=fitness(R_total,hj,yyj,...
                     z_total,number_slice,E_syn,delta_set(i,1));
                force_dif=(contact_force-force)/force;
                if abs(force_dif)<tolerance
                    flag=1;
                    right=i;
                    break;
        ...
        delta_route
        while judgment==0&&flag==0
            delta_route=ones(popsize,1);
            delta_set=init_delta(popsize,delta_set(popsize,1)*1.25)
            for i=1:popsize
                contact_force=fitness(R_total,hj,yyj,...
                     z_total,number_slice,E_syn,delta_set(i,1));
                force_dif=(contact_force-force)/force;
    ...
    if flag==0
        loc1=find(delta_route,1)-1;
        loc2=find(delta_route,1);
        temp=(delta_set(loc1,1)+delta_set(loc2,1))/2;
        delta_set=init_delta(popsize,temp);
    end
end 
 

%function [force1,force2]=select_delta(popsize,delta_outer_set,delta_inner_set,force)
%    for i=1:popsize-1
%       delta_outer1=delta_outer_set(i,1);
%       delta_outer2=delta_outer_set(i+1,1);
%       [contact_force_outer1,contact_stress_outer,a_i_outer,a_i_outer_first,a_i_outer_last,mark_outer]=linear_contact_inter(R_total_outer,hj,yyj,...
%                  z_total_outer,number_slice,E_roller_outer,delta_outer1);
%       [contact_force_outer2,contact_stress_outer,a_i_outer,a_i_outer_first,a_i_outer_last,mark_outer]=linear_contact_inter(R_total_outer,hj,yyj,...
%                  z_total_outer,number_slice,E_roller_outer,delta_outer2);
%       if contact_force_outer1>=force and  contact_force_outer2>=force
